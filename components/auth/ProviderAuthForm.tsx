...
export default function ProviderAuthForm({ type }: ProviderAuthFormProps) {
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            ...
            // Google sign in redirect is handled by Supabase auth settings, 
            // usually redirects to /auth/callback which should handle routing logic based on user state
        } catch (error) {
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 md:p-10">
                ...
                <div className="text-center mb-10">
                    ...
                    <h2 className="text-3xl font-bold text-gray-900 mb-2">
                        {type === "login" ? "Partner Login" : "Partner Signup"}
                    </h2>
                    <p className="text-gray-500">
                        {type === "login"
                            ? "Manage your offers and applications."
                            : "Create an account to start hiring creators."}
                    </p>
                </div>
                ...
                <button
                    onClick={async () => {
                        setIsLoading(true);
                        try {
                            await signInWithFacebook('provider');
                        } catch (error) {
                    ...
                    disabled={isLoading}
                    className="w-full h-12 bg-[#1877F2] hover:bg-[#166fe5] text-white font-bold rounded-xl transition-all flex items-center justify-center gap-3 disabled:opacity-70"
                >
                ...
                <div className="mt-8 text-center space-y-2">
                    <p className="text-gray-500 text-sm">
                        ...
                        <Link
                            href={type === "login" ? "/auth/provider/signup" : "/auth/provider/login"}
                    ...
                    <p className="text-gray-400 text-xs">
                        Are you a creator?{" "}
                        <Link href="/auth/login" className="font-medium text-gray-500 hover:text-gray-900 underline">
                            Login here
                        </Link>
                    </p>
                </div>
